...
switch ($page) {
    case 'dashboard':
        ...
        // 1. Get KPIs (UTC-safe)
        $kpi_data = getDashboardKpisUtc($pdo);

        // 2. Pending shift reviews（沿用原函数）
        $pending_shift_reviews_count = getPendingShiftReviewCount($pdo);

        // 3. Low stock alerts（沿用原函数）
        $low_stock_alerts = getLowStockAlerts($pdo, 10);

        // 4. Sales trend last 7 days（沿用原函数）
        $sales_trend = getSalesTrendLast7Days($pdo);

        // 5. Top products today (UTC-safe)
        $top_products = getTopSellingProductsTodayUtc($pdo);
